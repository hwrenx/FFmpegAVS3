...
static av_cold int uavs3d_init(AVCodecContext *avctx)
{
...
static av_cold int uavs3d_end(AVCodecContext *avctx)
{
    UAVS3DContext *h = avctx->priv_data;
...
static void uavs3d_flush(AVCodecContext * avctx)
{
    UAVS3DContext *h = avctx->priv_data;
    uavs3d_cfg_t cdsc;
    cdsc.frm_threads = FFMIN(h->frame_threads > 0 ? h->frame_threads : av_cpu_count(), UAVS3D_MAX_FRAME_THREADS);
...
static int uavs3d_decode_frame(AVCodecContext *avctx, void *data, int *got_frame, AVPacket *avpkt)
{
    UAVS3DContext *h = avctx->priv_data;
